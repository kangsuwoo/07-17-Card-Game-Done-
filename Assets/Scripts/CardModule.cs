...
public class CardModule : MonoBehaviour
{
    ...
    private IEnumerator UpdateRotate(bool isUp, float time)
    {
        ...
        //  ? -> 삼항 연산자
        //  조건 ? true : false;
        Quaternion prev = Quaternion.Euler(rotate.x, isUp ? 0 : 180, rotate.z);
        Quaternion next = Quaternion.Euler(rotate.x, isUp ? 180 : 0, rotate.z);
        //Quaternion prev = isUp ? Quaternion.identity : Quaternion.Euler(rotate.x, 180, rotate.z);
        //Quaternion next = isUp ? Quaternion.Euler(rotate.x, 180, rotate.z) : Quaternion.identity;
